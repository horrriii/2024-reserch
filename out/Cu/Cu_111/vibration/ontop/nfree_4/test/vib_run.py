...
vib = Vibrations(slab_structure, indices=[-1,-2], nfree=4)
write("check.traj",vib.iterimages()) # type: ignore
vib.iterdisplace(inplace=False)
for displacement, displacement_atoms in vib.iterdisplace():
    with open("cache."+ displacement(a) + displacement(i)+displacement(sign)+".json","w") as out_file:
        #TODO iterdisplace
# vib.run()
